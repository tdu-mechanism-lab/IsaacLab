...
def joint_default_deviation_l2(
    ...
    """デフォルト関節角からの逸脱をL2で罰する(姿勢アンカー)。
    calf2が寝る・胴がVの字になる等の「初期姿勢から大きく崩れた局所解」を抑制する。
    対象関節はasset_cfg.joint_namesで指定(例: ".*_calf2_joint", "spine_joint"）。
    """
    asset = env.scene[asset_cfg.name]
    angle = asset.data.joint_pos[:, asset_cfg.joint_ids]
    default = asset.data.default_joint_pos[:, asset_cfg.joint_ids]
    return torch.sum(torch.square(angle - default), dim=1)


def joint_velocity_uniformity(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    group_size: int = 3,
) -> torch.Tensor:
    """脚内の関節速度の偏りを罰する（仕事の分担を促す）。
    各脚の(thigh, calf, calf2)の|joint_vel|の分散を取り、
    「calfを固めてcalf2だけ動かす」「thighだけ回す」等の偏った解を抑制。
    joint_namesは脚ごとに連続するよう [FL_thigh, FL_calf, FL_calf2, FR_thigh, ...] の順で渡す。
    """
    asset = env.scene[asset_cfg.name]
    jvel = torch.abs(asset.data.joint_vel[:, asset_cfg.joint_ids])   # (N, 12)
    n_legs = jvel.shape[1] // group_size
    per_leg = jvel.reshape(jvel.shape[0], n_legs, group_size)        # (N, 4, 3)
    # 脚内での正規化分散（平均で割ることでスケール不変にし、種間で同条件に）
    mean = per_leg.mean(dim=2, keepdim=True).clamp(min=1e-4)
    var = ((per_leg - mean) / mean).square().mean(dim=2)             # (N, 4)
    return var.mean(dim=1)


def fore_hind_contact_balance(
    env: ManagerBasedRLEnv,
    sensor_cfg: SceneEntityCfg,
    command_name: str = "base_velocity",
) -> torch.Tensor:
    """前脚と後脚の接地時間バランスを報酬化（後脚だけ歩行の抑制）。
    sensor_cfg.body_namesは [FL_foot, FR_foot, RL_foot, RR_foot] の順(前2つ→後2つ)で渡す。
    前後それぞれの接地率の積を報酬にする: 両方使うと最大、片側だけだと0。
    静止コマンド時はほぼ全接地なので自然に飽和し悪さをしない。
    """
    contact_sensor: ContactSensor = env.scene[sensor_cfg.name]
    net_forces = contact_sensor.data.net_forces_w_history[:, :, sensor_cfg.body_ids, :]
    in_contact = (torch.norm(net_forces, dim=-1).max(dim=1)[0] > 1.0).float()  # (N, 4)
    fore = in_contact[:, :2].mean(dim=1)   # 前2脚の接地率
    hind = in_contact[:, 2:].mean(dim=1)   # 後2脚の接地率
    # 移動コマンドが出ているときのみ有効化
    cmd = torch.norm(env.command_manager.get_command(command_name)[:, :2], dim=1)
    return (fore * hind) * (cmd > 0.1).float()
